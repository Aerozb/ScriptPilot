...
// npm 对同一 prefix 并发安装会互相破坏 node_modules，用进程内互斥串行化。
...
export async function ensureScriptDependencies(input) {
  ...
  if (missing.length) {
    const pending = installQueue.then(() => installDependencies({
      paths: input.paths,
...
async function writeDependencyCache(paths, cache) {
  const pending = cacheWriteQueue.then(() =>
    writeFile(path.join(paths.appStateRoot, CACHE_FILE_NAME), `${JSON.stringify(cache, null, 2)}\n`, 'utf8')
  );
  cacheWriteQueue = pending.then(() => undefined, () => undefined);
  await pending;
}
