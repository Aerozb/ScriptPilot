...
export class JsonSettingsRepository {
  ...
  async get() {
    try {
      return mergeSettings(DEFAULT_SETTINGS, await this.store.read());
    } catch {
  ...
  async save(input) {
    // 串行化读改写并原子落盘，避免并发保存互相覆盖或写坏文件。
    const next = await this.store.update((current) => {
      const base = mergeSettings(DEFAULT_SETTINGS, current);
      return mergeSettings(base, input);
    });
    return next;
  }
}
